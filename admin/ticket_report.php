...
function exportPDF($data, $reportType) {
    ...
    $summary = [];
    if (!empty($data)) {
        if (in_array($reportType, ['4_status_breakdown', '4_status_breakdown_report'])) {
            $summary = [
                ['label' => 'Total Tickets', 'value' => array_sum(array_column($data, 'ticket_count'))],
            ];
        } elseif ($reportType === '8_resolution_time_analysis') {
            $summary = [
                ['label' => 'Categories', 'value' => count($data)],
                ['label' => 'Total Tickets', 'value' => array_sum(array_column($data, 'ticket_count'))],
            ];
        } elseif ($reportType === '9_geographic_distribution') {
            $summary = [
                ['label' => 'Provinces', 'value' => count($data)],
                ['label' => 'Total Tickets', 'value' => array_sum(array_column($data, 'total_tickets'))],
                ['label' => 'Sites', 'value' => array_sum(array_column($data, 'sites_count'))],
            ];
        } else {
            $summary = [
                ['label' => 'Total Tickets', 'value' => array_sum(array_column($data, 'total_tickets'))],
                ['label' => 'Open', 'value' => array_sum(array_column($data, 'open_tickets'))],
