...
sort($provinces);
// autocomplete sources from existing site data
$projStmt = $pdo->query("SELECT DISTINCT project_name FROM sites WHERE project_name IS NOT NULL AND project_name != '' ORDER BY project_name");
$allProjects = $projStmt->fetchAll(PDO::FETCH_COLUMN);
$ispStmt = $pdo->query("SELECT DISTINCT isp FROM sites WHERE isp IS NOT NULL AND isp != '' ORDER BY isp");
$allISP = $ispStmt->fetchAll(PDO::FETCH_COLUMN);

$locNameStmt = $pdo->query("SELECT DISTINCT location_name FROM sites WHERE location_name IS NOT NULL AND location_name != '' ORDER BY location_name");
$allLocationNames = $locNameStmt->fetchAll(PDO::FETCH_COLUMN);
$siteNameStmt = $pdo->query("SELECT DISTINCT site_name FROM sites WHERE site_name IS NOT NULL AND site_name != '' ORDER BY site_name");
$allSiteNames = $siteNameStmt->fetchAll(PDO::FETCH_COLUMN);
$apCodeStmt = $pdo->query("SELECT DISTINCT ap_site_code FROM sites WHERE ap_site_code IS NOT NULL AND ap_site_code != '' ORDER BY ap_site_code");
$allApCodes = $apCodeStmt->fetchAll(PDO::FETCH_COLUMN);
$munStmt = $pdo->query("SELECT DISTINCT municipality FROM sites WHERE municipality IS NOT NULL AND municipality != '' ORDER BY municipality");
$allMunicipalities = $munStmt->fetchAll(PDO::FETCH_COLUMN);
$provStmt = $pdo->query("SELECT DISTINCT province FROM sites WHERE province IS NOT NULL AND province != '' ORDER BY province");
...
$statusAllowed = array_map('strtoupper', $statusOptions);
...
function normalizeSiteStatus($status) {
    global $statusAllowed;
    $status = strtoupper(trim($status));
    return in_array($status, $statusAllowed, true) ? $status : false;
}
...
// duplicate checker used by both single and bulk operations
function isDuplicateSite($project, $location, $siteName, $apSiteCode, $province, $municipality, $excludeId = null) {
    global $pdo;
    $sql = "SELECT COUNT(*) FROM sites WHERE project_name=? AND location_name=? AND site_name=? AND ap_site_code=? AND province=? AND municipality=?";
    if ($excludeId !== null) {
        $sql .= " AND id<>?";
    }
    $stmt = $pdo->prepare($sql);
    if ($excludeId !== null) {
        $stmt->execute([$project, $location, $siteName, $apSiteCode, $province, $municipality, $excludeId]);
    } else {
        $stmt->execute([$project, $location, $siteName, $apSiteCode, $province, $municipality]);
    }
...
elseif ($action == 'view_details') {
    $id = $_POST['id'];
    // sites table stores province/municipality directly
...
elseif ($action == 'save') {
    ...
    try {
        ...
        if ($status === false) { echo 'Error: Invalid status.'; exit; }
...
elseif ($action == 'update') {
    ...
    try {
        ...
        if (isDuplicateSite($project, $location, $site, $apSiteCode, $province, $municipality, $id)) {
            echo 'Error: Duplicate site record.'; exit;
        }
        $project = Sanitizer::normalize($project);
        $location = Sanitizer::normalize($location);
...
elseif ($action == 'bulk_update') {
    ...
    try {
        ...
        $sql = "UPDATE sites SET $setClause WHERE id IN ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $params = array_merge(array_values($updates), $selectedIds);
        $stmt->execute($params);
...
foreach ($cols as $col) {
    if (!empty($_GET["filter_$col"])) {
        ...
        if ($col === 'status') {
            ...
            $vals = array_filter($vals, fn($v) => in_array($v, $statusAllowed, true));
...
if (!empty($searchQuery)) {
    ...
    // wrap the OR-conditions in parentheses so additional AND clauses
    // (such as hide-status) apply to the whole search expression instead
    // of only the last term due to SQL operator precedence.
    $whereClause = "WHERE (project_name LIKE ? OR status LIKE ? OR location_name LIKE ? OR site_name LIKE ? OR ap_site_code LIKE ? OR isp LIKE ? OR municipality LIKE ? OR province LIKE ?)";
    $params = array_fill(0, 8, $searchTerm);
}
// status is handled along with other column filters above; no separate hide logic needed
...
<script>
  ...
  // status filtering is now part of applyColumnFilters; old helper removed
