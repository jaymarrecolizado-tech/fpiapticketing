...
function exportPDF($data, $reportType) {
    ...
    $reportName = PdfGenerator::getReportName($reportType);
    $pdf = new PdfGenerator($reportName);
    $filename = str_replace(' ', '_', $reportName) . '_' . date('Y-m-d_H-i-s') . '.pdf';

    // Compute summary cards for key report types
    $summary = [];
    if (!empty($data)) {
        if ($reportType === '1_site_summary') {
            $summary = [
                ['label' => 'Total Sites', 'value' => count($data)],
                ['label' => 'Total Tickets', 'value' => array_sum(array_column($data, 'total_tickets'))],
                ['label' => 'Open Tickets', 'value' => array_sum(array_column($data, 'open_tickets'))],
                ['label' => 'Resolved', 'value' => array_sum(array_column($data, 'resolved_tickets'))],
            ];
        } elseif ($reportType === '3_isp_performance') {
            $summary = [
                ['label' => 'Total ISPs', 'value' => count($data)],
                ['label' => 'Total Tickets', 'value' => array_sum(array_column($data, 'total_tickets'))],
                ['label' => 'Open Tickets', 'value' => array_sum(array_column($data, 'open_tickets'))],
                ['label' => 'Avg Resolution (days)', 'value' => number_format(array_sum(array_column($data, 'avg_resolution_days')) / count($data), 1)],
            ];
        } elseif ($reportType === '8_monthly_activity') {
            $summary = [
                ['label' => 'Months', 'value' => count($data)],
                ['label' => 'Total New', 'value' => array_sum(array_column($data, 'new_tickets'))],
                ['label' => 'Total Resolved', 'value' => array_sum(array_column($data, 'resolved_tickets'))],
                ['label' => 'Sites Affected', 'value' => array_sum(array_column($data, 'sites_affected'))],
            ];
        }
    }

    // Build HTML and output
    $filters = $_GET['filters'] ? json_decode($_GET['filters'], true) : [];
    $html = $pdf->buildReport($data, [
